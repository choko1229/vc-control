...
6. ブラウザで `http://127.0.0.1:49162/setup` または設定した `DASHBOARD_BASE_URL` の `/setup` を開く
7. 以下を入力して保存する
   - Bot Token
   - Discord Client ID
...
9. 一度プロセスを再起動する
10. `http://127.0.0.1:49162/login` から Discord OAuth2 ログインする
...
- Web UI は FastAPI + Jinja2 構成のまま、Discord 風フラットUIの共通レイアウト / テーマ切替 / トースト / 統計演出を `vc_control/static/css/theme.css` `vc_control/static/css/discord-flat.css` と `vc_control/static/js/*.js` に分離しています。
- フォントは Google Fonts の `Noto Sans` を読み込みます。外部 CDN を使いたくない場合は、`base.html` のフォント読み込みをローカル配信へ差し替えてください。
